...
class NormalizedL1Loss(nn.Module):
    def __init__(self, alpha=0.0, eps=1e-8):
        super().__init__()
        self.alpha = alpha
        self.eps = eps
        self.l1 = nn.L1Loss()
    ...
    def forward(self, pred, target):
        abs_error = torch.abs(pred - target)          # |y - ŷ|
        
        mean_err = abs_error.mean()                    # μ
        std_err = abs_error.std() + self.eps           # σ + ε (防止除零)
        
        normalized_loss = ((abs_error ) / std_err.detach()).mean()
        l1_loss = self.l1(pred, target)
        
        return normalized_loss + self.alpha * l1_loss
...
class norm_loss_with_normalization(nn.Module):
    ...
    def forward(self, y_pred, y, alpha=[1, 0], p=2, q=2, detach=False, exponent=True):
